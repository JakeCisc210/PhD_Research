function [player_struct,profit] = mark_2_gambling_system(sample_values,x,opt)
    ...
    [moneyline_profit,win_condition_values,palpha_values,pbeta_values] = iterative_classical_profit_from_sample(sample_values,ones(1,num_values),opt.moneyline_tolerance,'profit_cutoff',1e-3);
    for index = 1:num_values
    ...
    for index = 1:(num_remaining/2)
        ...
        prob1 = sorted_remaining_sample_values(index);
        prob2 = sorted_remaining_sample_values(index+num_remaining/2);
        peer_bet = peer_to_peer(prob1,prob2,1,x);
    ...
    if profit_outside_in >= profit_split_n_pair
        ...
        for index = 1:(num_remaining/2)
            ...
            prob2 = sorted_remaining_sample_values(num_remaining/2+index);
            player2_number = sorted_remaining_player_numbers(num_remaining/2+index);
            assert(prob2 == player_struct(player2_number).player_probability)
